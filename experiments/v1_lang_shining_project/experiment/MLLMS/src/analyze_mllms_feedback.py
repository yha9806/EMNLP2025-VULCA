...
if __name__ == "__main__":
    BASE_DATA_DIRECTORY = "MLLMS/feedbacks/"
    ...
    for model_name in sorted(os.listdir(BASE_DATA_DIRECTORY)):
        ...
        if not os.path.isdir(model_dir):
            continue  # 跳过非目录项
        print(f"\n==== 开始分析模型: {model_name} ====")
        MODEL_FEEDBACK_DIR = model_dir
        OUTPUT_RESULTS_BASE_DIR = os.path.join(MODEL_FEEDBACK_DIR, "analysis_results")
        ...
        for root_dir, _, files in os.walk(MODEL_FEEDBACK_DIR):
            print(f"遍历目录: {os.path.abspath(root_dir)}")
        ...
        if not feedback_files_found:
            print(f"未找到.txt文件: {MODEL_FEEDBACK_DIR}")
            continue
        print(f"共发现{len(feedback_files_found)}个反馈文件，开始处理...")

        headers_written_files = set()
        for file_path in tqdm(feedback_files_found, desc=f"[{model_name}] 处理反馈文本"):
            relative_file_path = os.path.relpath(file_path, MODEL_FEEDBACK_DIR)
