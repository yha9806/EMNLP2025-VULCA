...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("run_hf_inference_qwen3_235b.log"),
...
def get_image_slices(monthly_image_folder_name):
    monthly_image_path = os.path.join(IMAGES_DIR, monthly_image_folder_name, "large")
    ...
    for item in os.listdir(monthly_image_path):
        if os.path.isfile(os.path.join(monthly_image_path, item)) and item.lower().endswith(('.png', '.jpg', '.jpeg', '.gif', '.bmp', '.webp')):
...
def encode_image_to_base64(image_path):
    try:
        ...
        logging.error(f"图片 base64 编码失败: {image_path}: {e}")
        return None
...
def run_experiment():
    ...
    pbar = tqdm(total=total_tasks, desc="Qwen3-235B 批量推理进度", ncols=100)
